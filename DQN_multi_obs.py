...
class AirSimEnv(gymnasium.Env):
    def __init__(self):
        ...
        # Set point
        self.spawn_setpoint = True  #Bool to include setpoint visualization
    ...
    def sim_initialization(self):
        """
        Sets up cylinder obstacles, spawns goal, spawns setpoint
        """
        # Stretch Cylinders
        for i in range(2, 9):
            name = "Cylinder" + str(i)
    ...
    def reset_goal(self):
        """
        Resets goal to its start position
        """
        temp_pose = airsim.Pose()
    ...
    def get_reward(self, collision_check):
        """
        ...
        if collision_check:
            reward = -50
        # If no collision, calculate reward based on x,y,z components
        else:
